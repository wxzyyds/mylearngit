...
//		给FileInputStream添加缓冲功能,从FileInputStream派生一个类：
//		BufferedFileInputStream extends FileInputStream
		
		
//		给FileInputStream添加计算签名的功能，类似的，也可以从FileInputStream派生一个类：	
...
//		给FileInputStream添加缓冲和签名的功能，那么我们还需要派生BufferedDigestFileInputStream。
//		BufferedDigestFileInputStream extends FileInputStream
		
		
//		如果要给FileInputStream添加缓冲和加解密的功能，则需要派生BufferedCipherFileInputStream。
//		BufferedCipherFileInputStream extends FileInputStream

	}
	/*
	 * 针对InputStream的子类设计，直接使用继承会子类爆炸，过度依赖导致子类数量失控，无法精确控制代码的复杂度，分类控制
	 */
	
	/*
	 * 提供数据基础的InputStream：FileInputStream，ByteArrayInputStream,ServletInputStream...	、
	 * 
	 * 1.确定数据源
	 * 2.使用InputStream引用来接收不同功能包装出的InputStream，
	 * InputStream file = new FileInputStream("test.gz");//接收源文件
	 * InputStream buffered = new BufferedInputStream(file);//设置源文件缓冲
	 * InputStream gzip = new GZIPInputStream(buffered);//读取已压缩的源文件
	 * 
	 * 这种通过基础组件，依次叠加附加功能的模式，叫做Filter模式，或者装饰器模式：Decorator
	 * 
	 * OutputStream同理，以Filter模式，装饰器模式来实现并提供多种功能
	 * 
	 * 
	 */
...
并且，
当最外层的InputStream关闭时（在try(resource)块的结束处自动关闭），
内层的InputStream的close()方法也会被自动调用，
并最终调用到最核心的“基础”InputStream，
因此不存在资源泄露。
	 */
